...
2、保持select脚为高电平，并向SD卡发送不低于74个时钟脉冲，此时sd卡将进入它的native operation mode（翻译为本地操作模式了），
	 并做好接收本地指令的准备。
3、此时将select脚拉低并向SD卡发送cmd0指令，SD卡在检测到select脚为低并收到cmd0指令后将进入spi模式，并返回0x01空闲状态。
4、成功进入空闲状态后，继续发送cmd8指令，携带的参数为0x000001AA，如果SD卡不能识别并返回错误码5，说明此卡非2.0卡（进入步骤6）。 
   如果指令能被识别，则SD卡将返回R1+32位共40位数据，其中R1位0x01,32位数据为0x000001AA。此时可判断卡为2.0卡。
5、判断为2.0卡后，紧接着发送cmd55+acmd41指令（应该是在cmd55返回1的情况下再接着发送acmd41），如果返回0，则初始化成功。此时如果
   你想继续判断是标准2.0卡SCSD还是大容量HCSD，则需要继续发送cmd58指令，并在cmd58指令被成功响应后连续从SD卡读取4个字节的ocr数据，
   若其第31位为1，表示初始化成功，其第30位为1表示是HCSD，为0表示SCSD。 此时2.0卡初始化结束。
6、步骤4对cmd8无响应，则继续判断卡为1.0或mmc卡。此时同步骤5一样向卡发送cmd55+acmd41指令，如果返回0，则表示1.0卡初始化成功，如果
   返回错误，则确定卡为mmc卡。 在确定卡为mmc卡后，继续向卡发送cmd1指令，如果返回0，则mmc卡初始化成功，否则失败，判断为错卡！！
7、初始化成功后，将spi切换为高速模式。
*/
...
static rt_err_t rt_msd_init(rt_device_t dev)
{
    ...
    /* config spi */
    {
        struct rt_spi_configuration cfg = RT_SD_CARD_SPI_CFG;
        //cfg.data_width = 8;
    ...
    /* init SD card */
    {
        ...
        else if (msd->card_type == MSD_CARD_TYPE_SD_V2_X)
        {
            ...
            result = _send_cmd(msd->spi_device, READ_OCR, 0x00, 0x01, response_r3, response);
...
//                    break;
    ...
    /* CMD16 SET_BLOCKLEN */
    {
        ...
        msd->geometry.block_size = SECTOR_SIZE*8;
...
//        result = _send_cmd(msd->spi_device, SEND_CSD, 0x00, 0xAF, response_r1, response);
        result = _send_cmd(msd->spi_device, SEND_CSD, 0x00, 0x01, response_r1, response);
